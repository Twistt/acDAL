...
namespace ArachnidCreations.DevTools
{
    public class IORM
    {
        ...
        private DataTable GetDT(string tablename)
        {
            ...
            if (dt == null)
            {
                dt = dal.getInstancedTableStructure(tablename);
                if (dt != null)
                {
        ...
        public T GetSingle<T>(string tablename, int id, string keyname)
        {
            return GetSingle<T>(tablename, id.ToString(), keyname);
        }

        public T GetSingle<T>(string tablename, string id, string keyname, string orderby = null)
        {
        ...
        public string GetSingleValue(string tablename, string key, string id, string column)
        {
            var str = string.Format("select top 1 {0} from {1} where {2} = '{3}'", column, tablename, key, id);
            DataTable dt = dal.Load(str);
            if (dt != null && dt.Rows.Count > 0)
            {
                var response =  dt.Rows[0][0].ToString();
                return response;
            }
            else return null;
        }
        ...
        public List<T> GetList<T>()
        {
            ...
            return GetList<T>(tablename, null);
        }
        ...
        /// <returns></returns>
        public List<T> GetList<T>(string tablename, Dictionary<string, string> where, bool or = false, int limit = 10000)
        {
        ...
        public List<T> convertDataTabletoObject<T>(DataTable dataTable)
        {
            int icount = 0;
        ...
        public T convertDataRowtoObject<T>(DataRow row, string fieldprefix = "")
        {
            int icount = 0;
            DataRow dt = row;
            var type = typeof(T);
        ...
        public T PopulateObjectFromObject<T>(object fromObject, object toObject)
        {
        ...
        public T ObjectFromNVP<T>(string tablename, DAL dal, int id = 0, string keyname = null)
        {
        ...
        public T GetSingle<T>(string tablename, int id)
        {
            var sql = string.Format("select * from {0} where {2} = '{1}'", tablename, id, GetPrimaryKey(tablename));
            DataTable dt = dal.Load(sql);
            int icount = 0;
            Type tType = typeof(T);
        ...
        public string InsertOrUpdate<T>(T userObj, string TableName, string whereClause, string PrimaryKey)
        ...
        public string generateInsertSQL<T>(List<T> objs, string tablename)
        {
            string pk = string.Empty;
            ...
            StringBuilder sql = new StringBuilder();
            //Select only the properties that are system types (they will match sql data types mostly) as a type of "User" wont match any sql field.
            List<PropertyInfo> props = typeof(T).GetProperties().Where(p => p.PropertyType.ToString().ToLower().Contains("system.") && p.CustomAttributes.Where(a => a.AttributeType.Name == "DBColumnIgnoreOnInsert").FirstOrDefault() == null).ToList();
            //props = props.Where(p => p.CustomAttributes.Where(a => a.AttributeType.Name == "DBColumnIgnoreOnInsert").FirstOrDefault() == null).ToList();
            foreach (var col in cols.Distinct())
            {
                //skip the id - we assume this is here for all tables
